...
namespace bnhe
{
	...
	void Box2DPhysicsComponent::Update(float delta)
	{
	...
	void Box2DPhysicsComponent::Read(const json::value_t& value)
	{
		...
		if (!shapeName.empty())
		{
			if (string::EqualsIgnoreCase(shapeName, "box")) m_bodyDef.shape = PhysicsBody::Shape::Box;
			else if (string::EqualsIgnoreCase(shapeName, "capsule")) m_bodyDef.shape = PhysicsBody::Shape::Capsule;
